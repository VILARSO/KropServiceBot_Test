...
async def init_db_connection():
    ...
    try:
        ...
        logging.info("Створено складений індекс на '(category, created_at)' для колекцію 'posts'.")
...
async def on_back_to_main(call: types.CallbackQuery, state: FSMContext):
...
async def on_back_to_prev_step(call: types.CallbackQuery, state: FSMContext):
...
async def add_type(call: types.CallbackQuery, state: FSMContext):
...
async def skip_cont(call: types.CallbackQuery, state: FSMContext):
...
async def view_start(call: types.CallbackQuery, state: FSMContext):
...
async def view_cat(call: types.CallbackQuery, state: FSMContext):
    idx = int(call.data.split('_')[2])
...
async def view_paginate(call: types.CallbackQuery, state: FSMContext):
...
async def my_posts_start(call: types.CallbackQuery, state: FSMContext):
...
async def my_posts_paginate(call: types.CallbackQuery, state: FSMContext):
...
async def debug_all_callbacks(call: types.CallbackQuery, state: FSMContext):
    current_state = await state.get_state()
